 ...
 */
export const passedThruKey = /*#__PURE__*/ Symbol('passed-thru-as');
...
export abstract class PassedThru<V, I = V> implements Iterable<I> {
   ...
   */
  abstract readonly [passedThruKey]: V;
   ...
   * @param target A value to check.
   *
   * @returns `true` if the `target` value is an object with a `[passedThruKey]` property, or `false` otherwise.
   */
  static is<V>(target: any): target is PassedThru<V, any>;

  static is(target: any): target is PassedThru<any, any> {
    return typeof target === 'object' && passedThruKey in target;
  }
   ...
   *
   * @returns A `[passedThruKey]` property value is the given `outcome` is a `PassedThru` instance, or `outcome` itself
   * otherwise.
   */
  static get<V>(outcome: V): PassedThru.Value<V> {
    if (PassedThru.is<V>(outcome)) {
      return outcome[passedThruKey] as PassedThru.Value<V>;
    }
